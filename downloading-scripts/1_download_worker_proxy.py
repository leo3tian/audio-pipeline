...
if not PROXY_URL:
    raise ValueError("Environment variable PROXY_URL must be set")
...
def get_proxy_url(logger=None):
    """
    Gets a proxy URL with optional session rotation.
    Format: http://user-<username>:<password>@<host>:<port>
    Note: Using http:// even for HTTPS connections, as the proxy may not support HTTPS itself
    """
    if not PROXY_URL:
    ...
    if '{RANDOM}' in proxy_url:
        # Generate a random session ID for proxy rotation
        session_id = random.randint(1, 1000000)
        return proxy_url.format(RANDOM=session_id)
    
    # Log proxy host (without credentials) for debugging
    if logger:
...
def download_audio(video_url: str, temp_dir: Path, worker_rank: int, logger: logging.Logger):
    ...
    proxy_url = get_proxy_url(logger)
    
    # Test proxy connection before download
    try:
        import requests
        ...
        r = requests.get('http://ip.oxylabs.io/location', proxies=proxies, timeout=10)
        if r.status_code == 200:
            logger.debug("✅ Proxy connection successful")
            try:
                response_data = r.json()
                if isinstance(response_data, dict):
                    logger.debug(f"Proxy IP: {response_data.get('ip', 'unknown')}")
                else:
                    logger.debug(f"Proxy response: {r.text[:100]}")
            except Exception as e:
                logger.debug(f"Could not parse proxy response: {r.text[:100]}")
        else:
            logger.warning(f"⚠️ Proxy test returned status code: {r.status_code}")
    except Exception as e:
        logger.warning(f"⚠️ Proxy test failed: {str(e)}")
    ...
    ydl_opts = {
        ...
        'throttledratelimit': MIN_DOWNLOAD_SPEED,
        
        # --- Error Handling ---
        'ignoreerrors': True,
        'verbose': True,  # Temporarily enable verbose output for debugging
